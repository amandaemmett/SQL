# For this project, I downloaded Bike Lane data from BreakIntoTech.
# Then I created a table to insert Bike Lane data into.
...
/*List each street and its average safety rating that averaged a 4 or higher.*/

WITH best_bike_lanes AS(
    SELECT cbl.street, AVG(cbl.safetyrating) as 'rating'
    FROM CityBikeLanes cbl
    GROUP BY cbl.street
    )
    SELECT street, rating, 'safe lane' as 'label'
    FROM best_bike_lanes
    WHERE rating >=4


/*List both safety ratings for second street, along with the average safety rating.*/
