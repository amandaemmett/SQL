...
SELECT payment_type, COUNT(id) Users, SUM(price) gross_profit
From sales
GROUP BY payment_type
ORDER BY Users DESC;


-What countries are customers located in?-
SELECT SUM(price) gross_profit, country
FROM sales
GROUP BY country
ORDER BY gross_profit DESC;

-In which state in the US is the most money being made?-
SELECT state, SUM(price) gross_profit
FROM sales
WHERE country= "United states"
GROUP BY state
ORDER BY gross_profit DESC;

-Cleaned the data to make it more uniform/easy to work with-
SELECT state, id
FROM sales
WHERE state="Michigan";


UPDATE sales SET state="MI" WHERE state="Michigan";
...
SELECT product, price, COUNT(id) Users
FROM sales
GROUP BY product
ORDER BY Users DESC;
